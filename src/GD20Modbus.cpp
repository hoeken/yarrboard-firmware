...
GD20Modbus::GD20Modbus(HardwareSerial& serial, int rxPin, int txPin)
    : _slaveID(0), _serial(serial), _rxPin(rxPin), _txPin(txPin) {}

void GD20Modbus::begin(uint8_t slaveID)
{
  _slaveID = slaveID;
...
float GD20Modbus::readOutputFreq()
{
  ...
  if (result == node.ku8MBSuccess) {
    ...
    YBP.println("⚠️ GD20: read frequency error");
    return -1;
  }
}
...
uint16_t GD20Modbus::readStatusWord()
{
  ...
  if (result == node.ku8MBSuccess) {
    ...
    YBP.println("⚠️ GD20: read status error");
    return 0xFFFF;
  }
}
...
{
  YBP.printf("GD20 Status: 0x%04X", status);

  if (status == 0xFFFF)
    YBP.println(" - Comms Error");
